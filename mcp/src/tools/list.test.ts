...
const createMockClient = (project?: string) =>
  ({
    query: vi.fn(),
    project,
...
describe('list tool handler', () => {
  it('maps status filter to backend values and formats output', async () => {
    ...
    const querySpy = mockClient.query as unknown as ReturnType<typeof vi.fn>;
  ...
  it('returns MCP error payload when status filter is invalid', async () => {
    ...
    const result = await listToolHandler(mockClient, { status: 'invalid' as never });
